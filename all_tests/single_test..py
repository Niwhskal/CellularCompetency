...
def run_ga (config, HW, restricted): 
    ...
    for g in tqdm(range(1, config['RUNS']+1)):
        ...
        if HW:
            ...
            SPopulation, _ = fcs.selection(C_population, CFitness, noevolution=True)
        ...
        #RFPopulation = fcs.mutate_singleInd(SPopulation)
        ...
        init_population = np.array(SPopulation).copy() #RFPopulation.copy() #stPopulation_stress.copy()
...
def plot_all(bubbles, fov_1, fov_10):
    ...
    for j in range(len(bubbles)):
        ...
        ax2.scatter(range(1, config['RUNS']+1), m_comp)#, label = 'Fov gene', marker='x')

        m_comp = np.mean(stress_gene_tracker[j, :, :], axis = 0)
        var_comp = np.std(stress_gene_tracker[j, :, :], axis = 0)/np.sqrt(config['Loops'])
        ax3.scatter(range(1, config['RUNS']+1), m_comp)#, label = 'Stress gene', marker='x')
    ...
    ax1.set(xlabel = 'Stages of swap', ylabel='Fitness')
    ax2.set(xlabel = 'Stages of swap', ylabel='Fov Gene Value of Best Individual')
    ax3.set(xlabel = 'Stages of swap', ylabel='Stress Gene value of Best Individual')
